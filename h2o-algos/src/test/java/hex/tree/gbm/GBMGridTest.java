...
public class GBMGridTest extends TestUtil {
  ...
  public void testCarsGrid() {
    ...
    try {
      ...
      final Grid<GBMModel.GBMParameters>.SearchFailure failures = grid.getFailures();
  ...
  public void testFailedParamsCleanup() {
    try {
      ...
      final Grid.SearchFailure emptyFailures = grid.getFailures();
      assertEquals(0, emptyFailures.getFailureCount());
      assertEquals(0, emptyFailures.getFailedParameters().length);
      assertEquals(0, emptyFailures.getFailedRawParameters().length);
      assertEquals(0, emptyFailures.getFailureDetails().length);
      assertEquals(0, emptyFailures.getFailureStackTraces().length);
